...
namespace DamageCalcChamp.Shared.Models
{
    public class DamageCalc
    {
        ...
        private long CorrectPower(ref PokemonMove move, PokemonDataReal atk, PokemonDataReal def)
        {
            ...
            // テラスタル中で、テラスタイプ一致であり、威力が60以下の技は威力を60に上げる
            // -> 先制技等は除外なので、本当はこの条件じゃダメ！！！
            if ( atk.Options[14] && atk.TeraType == move.Type && power <= 60 )
            {
                // 対戦でよく使われる実用的な技のほとんどは対象外なので、一旦処理自体を無効化する
                // グラススライダー、ほっぺすりすり、アシッドボム、クリアスモッグあたりは個別対応しておいても良いかも…
                if ( move.Name == "グラススライダー"
                    || move.Name == "ほっぺすりすり"
                    || move.Name == "アシッドボム"
                    || move.Name == "クリアスモッグ"
                    )
